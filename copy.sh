...
if [ -d "$HOME/.config/hypr/scripts" ] && [ -n "$(ls -A "$HOME/.config/hypr/scripts" 2>/dev/null)" ]; then
  chmod +x "$HOME/.config/hypr/scripts/"* 2>&1 | tee -a "$LOG"
fi
if [ -d "$HOME/.config/hypr/UserScripts" ] && [ -n "$(ls -A "$HOME/.config/hypr/UserScripts" 2>/dev/null)" ]; then
...
if command -v wallust >/dev/null 2>&1; then
  ...
  echo "${NOTE} wallust not found; skipping color scheme generation." 2>&1 | tee -a "$LOG"
fi
